...
> **Status 2026-07-11:** item 1 ✅ **implemented in code** — a "✦ Suggest actions" button in the actions toolbar sends the note body to `callClaude()` with `output_config.format` (a `json_schema` for `{actions: [{text, assignee}]}`); results render as dismissable/acceptable `suggestion-chip` rows above the toolbar, and accepting one appends it to `editActions` via the existing `markDirty()`/autosave path. ⚠ not yet verified live (needs a signed-in session + a BYOK key to exercise).
