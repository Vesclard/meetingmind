...
> **Status 2026-07-10:** item 1 ✅ **done, deployed, and verified live.** BYOK rules published to the console; the per-note code is on `main`/Vercel; owner confirmed the blob→per-note migration and CRUD round-trip cleanly in production. Item 2 ✅ **done & verified live (2026-07-10)** — transactional `updatedAt` conflict *detection* in `saveNoteDoc` (`noteVersions` map + `tsEqual`), a sync-conflict modal with Keep-mine (`forceSaveNoteDoc`) / Use-theirs (`reloadConflictNote`); owner confirmed with the two-browser test. Item 3 ✅ **done & verified live (2026-07-10)** — one shared write path (`persistCurrentNote`) drives both the Save button and a ~2s debounced autosave; `markDirty`/`editSeq`/`savedSeq` track dirty state (robust to edits made during an in-flight save), `commitPendingEdits` flushes on note/folder switch and sign-out, a `beforeunload` guard warns while dirty, and `setStatus` shows real states (Unsaved… / Saving… / Saved ✓ / ⚠ Offline). New notes autosave only once they have a title + project. Items 4–5 ⬜ not started.
