...
- **Reset app (Phase 2.4, live & verified 2026-07-10):** a "Reset app" button in the sidebar footer opens a danger confirm modal; `resetAppData` deletes every note doc (batched), reseeds `DEFAULT_DATA` via `writeAllPerNote`, and reloads. The legacy blob backup at `users/{uid}` is left untouched.
- **Loading skeleton (Phase 2.5, in code, pending live verification):** an `isLoading` flag makes `renderFolders`/`renderNoteList` emit paper-toned skeleton blocks during the post-sign-in `loadUserData` fetch (set true around the fetch in `onAuthStateChanged`), replacing the blank flash. A gentle pulse cues loading; the global reduced-motion rule stops it after one cycle.
