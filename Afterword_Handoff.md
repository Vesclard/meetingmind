...
- **Reset app (Phase 2.4, in code, pending live verification):** a "Reset app" button in the sidebar footer opens a danger confirm modal; `resetAppData` deletes every note doc (batched), reseeds `DEFAULT_DATA` via `writeAllPerNote`, and reloads. The legacy blob backup at `users/{uid}` is left untouched.
- **Still open:** a post-sign-in loading skeleton (Masterplan Phase 2 item 5).
