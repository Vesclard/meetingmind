...
- **Security rules:** the canonical, version-controlled source is now **`firestore.rules`** in the repo root — **mode: production/BYOK** (open sign-in, per-uid isolation, no owner email pin), chosen 2026-07-10. It uses a recursive `match /users/{uid}/{document=**}` scoped to `request.auth.uid == uid`, which covers the legacy `users/{uid}` blob *and* the Phase 2.1 subcollections (`meta/main`, `notes/{noteId}`):
  ...
  ```
  ⚠ **Not auto-deployed** (no Firebase CLI here) — the owner must publish `firestore.rules` in the Firebase console; **the Phase 2.1 code fails to save until this is published**, because the previously-deployed rule matched only `users/{uid}` (no subcollections). Per-document scoping to `request.auth.uid == uid` is non-negotiable: an earlier version of this setup used an unscoped `match /{document=**} { allow ... if request.auth != null; }` rule that let *any* signed-in Google account read/write *every* user's document — caught and fixed before real use. Before a public announcement, add Firebase App Check + per-note field-size caps (see `firestore.rules` header, afterword-security §6).
- **Data shape (⚠ migrated in code — deploy-gated on a Firestore rules update, 2026-07-10):** The app **code** now uses a **per-note model** (Masterplan Phase 2.1): folders in `users/{uid}/meta/main` (`{ folders }`), and one document per note at `users/{uid}/notes/{noteId}` (`{ ...note, updatedAt: serverTimestamp, schemaVersion: 2 }`). `loadUserData` reads the per-note store first; if it's empty it migrates the legacy blob into it (keeping the blob at `users/{uid}` as a one-release backup marked `migratedAt`) or seeds `DEFAULT_DATA`. `saveNote`/`deleteNote` are single-doc writes/deletes; `confirmAddFolder` writes `meta` only; `importData` batch-writes. **This code cannot function until the deployed Firestore rules cover the new subcollection paths** — the current rule matches only `users/{uid}` (below). Add a recursive `match /{document=**}` under `users/{uid}` with the same `request.auth.uid == uid` condition before deploying.
